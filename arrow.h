...
public:
	...
	virtual void paint(const graphic* g, const trans* t, bool drawconnectpoint, UINT64 generation, const point* offset = nullptr) const override { // ‘‚«’¼‚·•K—v‚ ‚è
	...
	virtual bool hit(const graphic* g, const point* p, UINT64 generation, const float offset) const override { // ‘‚«’¼‚·•K—v‚ ‚è
		if (isalive(generation) && start && end) {
	...
	virtual bool inrect(const point* p1, const point* p2, UINT64 generation, const float offset = 16.0f) const override { // ‘‚«’¼‚·•K—v‚ ‚è
		if (isalive(generation) && start && end) {
			if (
